...
export const getAllNews = createAsyncThunk(
  ...
  async  ({  country,page,category }: {  country: string,page:number,category:string}) => {
    console.log(category,"CATRGORY")
     const response = await axios.get(`https://api.thenewsapi.com/v1/news/top?api_token=wnaFPrt95uF8ttK8H2PXe9wlQfGuLjSaPCouGzZV&locale=${country}&limit=3&categories=${category}&page=${page}`);
